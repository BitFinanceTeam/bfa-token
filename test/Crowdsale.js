...
contract('TestCrowdSale', function(accounts) {
  it("Crowdsale & Token deployed", async function() {
    ...
    // console.log("TOKEN: inst.address", crowdsaleAddy);
  ...
  it("sets the release agent", async function() {
    await token.setReleaseAgent(inst.address, {from: _wallet})
    let agent = await token.releaseAgent.call()
  ...
  it("Crowdsale has correct owner", async function(){
    // console.log("Address 0 ", await accounts[0])
    var owner = await inst.owner.call()
    console.log("OWNER", owner)
    console.log("inst.address", inst.address)
    assert.equal(owner, _wallet)
  })
