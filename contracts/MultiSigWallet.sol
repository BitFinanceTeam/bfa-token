...
contract MultiSigWallet {
    ...
    modifier transactionExists(uint transactionId) {
        if (transactions[transactionId].destination == 0) revert();
        _;
    }
    ...
    modifier validRequirement(uint ownerCount, uint _required) {
        if (ownerCount > MAX_OWNER_COUNT) revert();
        if (_required > ownerCount) revert();
        if (_required == 0) revert();
        if (ownerCount == 0) revert();
        _;
    }
